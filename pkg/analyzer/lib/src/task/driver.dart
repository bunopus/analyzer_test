...
final DEBUG_LOGGER = new logging.Logger('DebugLogger');
const EXAMINED_ASSET_NAME = 'tmp_class.dart';
...
//    print('${'\t' * level}$spawningResult of $actualTarget');
  ...
  WorkItem gatherInputs(TaskManager taskManager, List<WorkItem> skipInputs) {
    while (builder != null) {
      ...
      final isExaminedInput = DO_LOGGING && (inputTarget.librarySource?.source?.shortName == EXAMINED_ASSET_NAME && inputResult == RESOLVED_UNIT1);
      if (isExaminedInput) {
        DEBUG_LOGGER.warning('!!! Gather Input requires RESOLVED_UNIT1 for $target');
      }
      CacheEntry inputEntry = context.getCacheEntry(inputTarget);
      if (isExaminedInput && !FLUSHED_ONCE && inputEntry.getState(inputResult) == CacheState.VALID) {
        context.analysisCache.flush((target, result) => target == inputTarget && result == RESOLVED_UNIT1);
        DEBUG_LOGGER.warning('!!! Flushing result for $target once');
        FLUSHED_ONCE = true;
        //We are not interested in other executions
        DO_LOGGING = false;
      }
      CacheState inputState = inputEntry.getState(inputResult);
      if (isExaminedInput) {
        DEBUG_LOGGER.warning('!!! Cache state is $inputState');
      }
